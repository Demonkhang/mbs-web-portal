...
export const AdminFeedbackPage: React.FC<AdminFeedbackPageProps> = ({ onNavigate, subTab = 'feedback' }) => {
  ...
  // Feedbacks State (ONLY Environmental Complaints)
  ...
  // Fetch Environmental Complaints ONLY from PostgreSQL CSDL via API
    ...
    try {
      ...
      if (res && res.data) {
        const mapped = res.data.map((item) => ({
          ...
          title: item.title || item.content || 'Phản ánh môi trường',
          applicant: `${item.senderName} (${item.senderPhone})`,
          facility: item.facility || 'Khu LHXLCT Đa Phước (Bình Chánh)',
          currentStep: item.currentStep || 1,
          assignedOfficer: item.assignedOfficer || 'Chưa phân công',
          department: item.assignedDepartment || 'Tổ Trực ban 24/7',
        ...
        if (mapped.length > 0 && !selectedSubId) {
          setSelectedSubId(mapped[0].id);
        }
      }
    } catch (error: any) {
      showToast('Lỗi kết nối', 'Không thể tải danh sách phản ánh từ CSDL PostgreSQL', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmAssign = async () => {
    ...
    const deptName = selectedOfficer.includes('(') ? selectedOfficer.split('(')[1].replace(')', '') : 'Ban Quản lý';

    try {
      await fetchApi(`/v1/inquiries/feedback/${currentSub.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          assignedOfficer: officerName,
          assignedDepartment: deptName,
          currentStep: currentSub.currentStep === 1 ? 2 : currentSub.currentStep,
          statusText: `Đã phân công ${officerName} (${deptName}) thụ lý`,
        }),
      });

      setIsAssignModalOpen(false);
      showToast('Phân công thành công!', `Đã giao phản ánh ${currentSub.code} cho ${officerName}`, 'success');
      loadSubmissions();
    } catch (error: any) {
  ...
  const handleSetStep = async (stepNumber: number) => {
    ...
    const statusText =
      stepNumber === 1
        ? 'Đã tiếp nhận hồ sơ'
        : stepNumber === 2
        ? 'Đang thẩm định hồ sơ & xác minh hiện trường'
        : stepNumber === 3
        ? 'Đang trình Lãnh đạo phê duyệt'
        : 'Đã hoàn tất & xuất bản công khai';
    ...
    try {
      ...
      showToast('Cập nhật tiến độ', `Phản ánh ${currentSub.code} đã chuyển sang Bước ${stepNumber}/4`, 'info');
      loadSubmissions();
    } catch (error: any) {
      showToast('Lỗi cập nhật', error.message || 'Không thể lưu vào CSDL', 'error');
    }
  ...
  const handleApprovePublish = async () => {
    if (!currentSub.id) return;
    if (!isAssignedOfficer) {
      showToast('Giới hạn quyền', `Chỉ cán bộ được phân công mới có quyền phê duyệt kết quả`, 'warning');
      return;
    }
    ...
    try {
      await fetchApi(`/v1/inquiries/feedback/${currentSub.id}`, {
        ...
        body: JSON.stringify({
          ...
          officialResponse: currentSub.officialResponse || 'Đã kiểm tra đột xuất hiện trường và xử lý xong.',
          isPublic: true,
        }),
      });

      showToast('Duyệt thành công!', `Đã công khai kết quả phản ánh ${currentSub.code} lên Cổng thông tin`, 'success');
      loadSubmissions();
    } catch (error: any) {
  ...
  const handleConfirmReject = async () => {
    if (!currentSub.id || !rejectionReason.trim()) {
      showToast('Cảnh báo', 'Vui lòng nhập lý do từ chối đơn phản ánh', 'warning');
      return;
    }

    try {
      const rejectNote = `Từ chối đơn: ${rejectionReason.trim()}`;
      await fetchApi(`/v1/inquiries/feedback/${currentSub.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: 'TU_CHOI',
          statusText: rejectNote,
          officialResponse: `[BỊ TỪ CHỐI] ${rejectNote}`,
          isPublic: false,
        }),
      });
      ...
      setRejectionReason('');
      showToast('Đã từ chối đơn', `Phản ánh ${currentSub.code} đã chuyển sang trạng thái Từ chối. Nút xóa đơn đã sẵn sàng!`, 'warning');
      loadSubmissions();
    } catch (error: any) {
      showToast('Lỗi từ chối', error.message || 'Không thể từ chối đơn', 'error');
    }
  };
  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      ...
      showToast('Đã xóa vĩnh viễn', `Phản ánh ${currentSub.code} đã được xóa sạch khỏi CSDL PostgreSQL`, 'success');
      setSelectedSubId('');
      loadSubmissions();
    } catch (error: any) {
      showToast('Lỗi xóa phản ánh', error.message || 'Không thể xóa phản ánh', 'error');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl flex flex-wrap items-center justify-between gap-3 shadow-lg">
        <div className="flex items-center gap-2 text-xs text-slate-300">
          ...
          <span className="px-2.5 py-1 rounded-lg bg-teal-950 text-teal-300 border border-teal-800 font-bold">
            {activeAccount}
          </span>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2.5">
            ...
            Quản lý Phản ánh - Kiến nghị Môi trường 24/7
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Tiếp nhận & xử lý thông tin phản ánh ô nhiễm môi trường, mùi hôi, nước rỉ rác từ người dân & doanh nghiệp
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="border-teal-800 text-teal-300 bg-teal-950/60 font-mono text-xs px-3 py-1">
            Phản ánh Môi trường: {submissions.length}
          </Badge>
        </div>
      </div>
        ...
        <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-3xl p-4 md:p-5 space-y-4 shadow-xl">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              ...
              Danh sách Phản ánh Môi trường ({submissions.length})
            </h3>
          </div>

          {isLoading ? (
            <div className="py-12 text-center text-xs text-slate-400">
              Đang tải danh sách phản ánh từ CSDL PostgreSQL...
            </div>
          ) : submissions.length === 0 ? (
            <div className="py-12 text-center text-xs text-slate-500">
              Chưa có phản ánh nào trong CSDL.
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl border transition-all cursor-pointer space-y-2.5 relative group ${
                      isSelected
                        ? 'bg-teal-950/40 border-teal-500/80 shadow-lg ring-1 ring-teal-500/30'
                        : 'bg-slate-950 border-slate-800/80 hover:border-slate-700'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-mono text-xs font-bold text-teal-400 bg-teal-950 px-2.5 py-0.5 rounded-lg border border-teal-800">
                        {sub.code}
                      </span>
                      {isSubRejected ? (
                        ...
                        <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-slate-800 text-teal-300 border border-slate-700">
        ...
        {currentSub && currentSub.id ? (
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 shadow-xl">
            ...
            <div className="border-b border-slate-800 pb-4 space-y-2">
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">ĐANG XEM PHẢN ÁNH MÔI TRƯỜNG</div>
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <h2 className="text-xl font-black text-teal-400 font-mono tracking-tight">{currentSub.code}</h2>
              ...
              {currentSub.location && (
                <p className="text-xs text-slate-400 flex items-center gap-1">
                  ...
                  <span>Địa điểm hiện trường: <strong className="text-slate-200">{currentSub.location}</strong></span>
                </p>
              )}
            </div>
            ...
            {!isAssignedOfficer && (
              <div className="p-3.5 bg-amber-950/40 border border-amber-800/80 rounded-2xl text-xs text-amber-200 flex items-center gap-2.5">
                ...
                <span>
                  <strong>Quyền hạn hạn chế:</strong> Phản ánh này hiện được giao cho{' '}
            ...
            <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="text-xs text-slate-300 space-y-1">
                ...
                <span className="text-[11px] text-teal-400 block">{currentSub.department}</span>
              </div>
              ...
              >
                <UserPlus className="w-4 h-4 text-teal-400" /> Phân công chuyên viên
              </Button>
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-slate-800">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-300 block">Nội dung Phản hồi / Kết quả Xử lý:</label>
                {isAssignedOfficer ? (
              ...
              <textarea
                ...
                placeholder={
                  isAssignedOfficer
                    ? 'Nhập nội dung trả lời công khai hoặc ghi chú thẩm định hiện trường...'
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-850">
                ...
                <Button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold gap-1.5 cursor-pointer shadow-lg disabled:opacity-40"
                >
                  <CheckCircle2 className="w-4 h-4" /> Phê duyệt & Công khai Kết quả
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center text-slate-500 text-xs">
            Vui lòng chọn một phản ánh môi trường từ danh sách bên trái.
          </div>
        )}
      </div>
          ...
          <div className="bg-white text-slate-900 rounded-2xl p-6 max-w-md w-full space-y-5 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-slate-700 flex items-center justify-between">
                ...
                <span className="text-[10px] text-teal-700 font-mono font-bold">Lấy từ CSDL PostgreSQL</span>
              </label>
              <select
                value={selectedOfficer}
                onChange={(e) => setSelectedOfficer(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 text-xs text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-teal-500 cursor-pointer"
              >
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <Button
                ...
                className="bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs cursor-pointer"
              >
                Xác nhận phân công
              </Button>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 text-white rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-start justify-between pb-3 border-b border-slate-800">
              <div>
                <h3 className="text-base font-bold text-white flex items-center gap-2">
                  <XCircle className="w-5 h-5 text-rose-400" /> Từ chối Phản ánh & Ghi lý do
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  Mã phản ánh: <strong className="text-teal-400 font-mono">{currentSub.code}</strong>
                </p>
              </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="VD: Phản ánh không có căn cứ thực tế, vị trí ngoài phạm vi quản lý của Ban Quản lý MBS..."
